...
import { type TagToken, TokenKind, Tokenizer } from "liquidjs";
import type {
	FilterToken,
	IdentifierToken,
	OutputToken,
	PropertyAccessToken,
} from "liquidjs/dist/tokens";
import {
	type CompletionItem,
	CompletionItemKind,
...
import {
	...
	PortalObjects,
	PortalTags,
} from "../constants/PortalEnums";
import type { ILiquidRuleEngineContext } from "./LiquidAutoCompleteRuleEngine";
import {
	type IManifestElement,
...
const getSuggestionsForEntity = (
	entityName: PortalEntityNames,
	ctx: ILiquidRuleEngineContext,
	textForAutoComplete?: string,
	insertById?: boolean,
): CompletionItem[] => {
	const partialMatch =
		textForAutoComplete
			?.replace(QUOTES_REGEX, "")
			.replace(AUTO_COMPLETE_PLACEHOLDER, "") || "";
	// return list of web templates for auto completion
	const matchedManifestRecords: IManifestElement[] =
		getMatchedManifestRecords(
			ctx.workspaceRootFolders,
			entityName,
...
const snippetObjectRule: ILiquidAutoCompleteRule = {
	name: "snippetObject",
	isValid: (liquidToken) =>
		liquidToken.kind === TokenKind.Output &&
		liquidToken.content.includes(PortalObjects.SNIPPETS),
	priority: DEFAULT_TAG_PRIORITY,
	apply: (liquidToken, ctx) => {
		return portalObjectBaseRule(
			liquidToken as OutputToken,
			PortalEntityNames.CONTENT_SNIPPET,
			ctx,
		);
	},
};
...
const settingsObjectRule: ILiquidAutoCompleteRule = {
	...
	isValid: (liquidToken) =>
		liquidToken.kind === TokenKind.Output &&
		liquidToken.content.includes(PortalObjects.SETTINGS),
	priority: DEFAULT_TAG_PRIORITY,
	apply: (liquidToken, ctx) => {
		return portalObjectBaseRule(
			liquidToken as OutputToken,
			PortalEntityNames.SITE_SETTING,
			ctx,
		);
	},
};
...
const weblinksObjectRule: ILiquidAutoCompleteRule = {
	...
	isValid: (liquidToken) =>
		liquidToken.kind === TokenKind.Output &&
		liquidToken.content.includes(PortalObjects.WEBLINKS),
	priority: DEFAULT_TAG_PRIORITY,
	apply: (liquidToken, ctx) => {
		return portalObjectBaseRule(
			liquidToken as OutputToken,
			PortalEntityNames.WEBLINK_SET,
			ctx,
		);
	},
};
...
const sitemakerObjectRule: ILiquidAutoCompleteRule = {
	...
	isValid: (liquidToken) =>
		liquidToken.kind === TokenKind.Output &&
		liquidToken.content.includes(PortalObjects.SITEMARKER),
	priority: DEFAULT_TAG_PRIORITY,
	apply: (liquidToken, ctx) => {
		return portalObjectBaseRule(
			liquidToken as OutputToken,
			PortalEntityNames.SITE_MARKER,
			ctx,
		);
	},
};
...
const rootObjectAttributesRule: ILiquidAutoCompleteRule = {
	...
	apply: (liquidToken) => {
		...
		if (
			...
			portalObject &&
			PORTAL_OBJECTS.includes(portalObject)
		) {
			suggestions.push(
				...(OBJECT_ATTRIBUTES_MAP.get(portalObject) ?? []).map(
					(key) => {
						return {
							label: key,
							insertText: key,
...
const entityFormTagRule: ILiquidAutoCompleteRule = {
	name: "entityFormTag",
	isValid: (liquidToken) =>
		liquidToken.kind === TokenKind.Tag &&
		(liquidToken as TagToken).name.toLowerCase() === PortalTags.ENTITYFORM,
	priority: DEFAULT_TAG_PRIORITY,
	apply: (liquidToken, ctx) => {
		const suggestions: CompletionItem[] = [];
...
const entityListTagRule: ILiquidAutoCompleteRule = {
	...
	isValid: (liquidToken) =>
		liquidToken.kind === TokenKind.Tag &&
		(liquidToken as TagToken).name.toLowerCase() === PortalTags.ENTITYLIST,
	priority: DEFAULT_TAG_PRIORITY,
	apply: (liquidToken, ctx) => {
		const suggestions: CompletionItem[] = [];
		const tokenizer = new Tokenizer((liquidToken as TagToken).args);
...
const webFormTagRule: ILiquidAutoCompleteRule = {
	...
	isValid: (liquidToken) =>
		liquidToken.kind === TokenKind.Tag &&
		(liquidToken as TagToken).name.toLowerCase() === PortalTags.WEBFORM,
	priority: DEFAULT_TAG_PRIORITY,
	apply: (liquidToken, ctx) => {
		const suggestions: CompletionItem[] = [];
		const tokenizer = new Tokenizer((liquidToken as TagToken).args);
...
const includeTagRule: ILiquidAutoCompleteRule = {
	...
	isValid: (liquidToken) =>
		liquidToken.kind === TokenKind.Tag &&
		(liquidToken as TagToken).name.toLowerCase() === PortalTags.INCLUDE,
	priority: DEFAULT_TAG_PRIORITY,
	apply: (tagToken, ctx) => {
		const suggestions: CompletionItem[] = [];
		...
		if (templateName.includes(AUTO_COMPLETE_PLACEHOLDER)) {
			suggestions.push(
				...getSuggestionsForEntity(
					PortalEntityNames.WEB_TEMPLATE,
					ctx,
					templateName,
				),
			);
		...
		hashes.forEach((hash) => {
			...
			if (
				...
				hashValue?.includes(AUTO_COMPLETE_PLACEHOLDER) &&
				templateName.replace(QUOTES_REGEX, "").toLowerCase() ===
					PortalAttributeNames.SNIPPET
			) {
				suggestions.push(
					...getSuggestionsForEntity(
						PortalEntityNames.CONTENT_SNIPPET,
				...
				hashValue?.includes(AUTO_COMPLETE_PLACEHOLDER) &&
				templateName.replace(QUOTES_REGEX, "").toLowerCase() ===
					PortalAttributeNames.ENTITY_LIST
			) {
				suggestions.push(
					...getSuggestionsForEntity(
						PortalEntityNames.ENTITY_LIST,
...
const editableTagRule: ILiquidAutoCompleteRule = {
	...
	isValid: (liquidToken) =>
		liquidToken.kind === TokenKind.Tag &&
		(liquidToken as TagToken).name.toLowerCase() === PortalTags.EDITABLE,
	priority: DEFAULT_TAG_PRIORITY,
	apply: (tagToken, ctx) => {
		const suggestions: CompletionItem[] = [];
		const tokenizer = new Tokenizer((tagToken as TagToken).args);
