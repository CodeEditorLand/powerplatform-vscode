...
import {
	...
	InvalidResponse,
	MalaciousScenerioResponse,
	NetworkError,
	PROMPT_LIMIT_EXCEEDED,
	PromptLimitExceededResponse,
	RELEVANCY_CHECK_FAILED,
	RateLimitingResponse,
	UnauthorizedResponse,
	type UserPrompt,
} from "./constants";
import type { IActiveFileParams } from "./model";
import { sendTelemetryEvent } from "./telemetry/copilotTelemetry";
import {
	CopilotResponseFailureEvent,
	CopilotResponseFailureEventWithMessage,
	CopilotResponseOkFailureEvent,
	CopilotResponseSuccessEvent,
		...
		if (response.ok) {
			try {
				...
				if (jsonResponse.operationStatus === "Success") {
					sendTelemetryEvent(telemetry, {
						eventName: CopilotResponseSuccessEvent,
				...
				sendTelemetryEvent(telemetry, {
					eventName: CopilotResponseOkFailureEvent,
					copilotSessionId: sessionID,
			...
			try {
				...
				if (response.status === 429) {
					...
					errorCode === RELEVANCY_CHECK_FAILED ||
					errorCode === INAPPROPRIATE_CONTENT ||
					errorCode === INPUT_CONTENT_FILTERED
				) {
					return MalaciousScenerioResponse;
				} else if (
					errorCode === PROMPT_LIMIT_EXCEEDED ||
					errorCode === INVALID_INFERENCE_INPUT
				) {
				...
				sendTelemetryEvent(telemetry, {
					eventName: CopilotResponseFailureEvent,
					copilotSessionId: sessionID,
					responseStatus: String(response.status),
					error: error as Error,
					durationInMills: responseTime,
