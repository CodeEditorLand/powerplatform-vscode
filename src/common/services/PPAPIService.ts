...
export class PPAPIService {
		...
		try {
			...
			if (response.ok) {
				...
				sendTelemetryEvent(telemetry, {
					eventName:
						VSCODE_EXTENSION_PPAPI_GET_WEBSITE_BY_ID_COMPLETED,
					orgUrl: websiteDetails.dataverseInstanceUrl,
				});
	...
	static async getPPAPIServiceEndpoint(
		serviceEndpointStamp: ServiceEndpointCategory,
		telemetry: ITelemetry,
		environmentId: string,
		websitePreviewId?: string,
		...
		return (
			PPAPI_WEBSITES_ENDPOINT.replace("{rootURL}", ppapiEndpoint).replace(
				"{environmentId}",
				environmentId,
			) +
