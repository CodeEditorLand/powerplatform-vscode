 ...
 */
export interface ECSFeatureDefinition<
	TConfig extends Record<string, string | boolean>,
...
> {
	 ...
	 */
	extractECSFeatureFlagConfig<TTeamConfig extends TConfig>(
		config: TTeamConfig,
	): Partial<TConfig>;
}

export type ECSFeatureInfo<
	TConfig extends Record<string, boolean | string>,
	TeamName extends string,
> = Omit<
 ...
 */
export function createECSFeatureDefinition<
	TConfig extends Record<string, boolean | string>,
	TeamName extends string,
>(
	...
	return {
		...featureInfo,
		extractECSFeatureFlagConfig: (config) =>
			extractECSFeatureFlagConfig(
				config,
				Object.keys(featureInfo.fallback),
			),
	};
}
 ...
 */
function extractECSFeatureFlagConfig<
	TConfig extends Record<string, boolean | string>,
	TeamConfig extends TConfig,
>(teamConfig: TeamConfig, keys: (keyof TConfig)[]): Partial<TConfig> {
