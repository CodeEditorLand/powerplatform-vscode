...
export function getPowerPageEntityType(
	uriPath: string,
): Constants.PowerPagesEntityType {
	...
	Constants.EntityFolderName.forEach((folderName) => {
		...
		if (uriPath.includes(`/${folderName}/`)) {
			pagesEntityType =
				Constants.EntityFolderMap.get(folderName) ??
...
export function getDeletePathUris(
	uriPath: string,
	...
	if (isValidUri(uriPath) && fileProperties.fileName) {
		if (fileEntityType === Constants.PowerPagesEntityType.WEBFILES) {
			...
			ymlExtensionIndex === -1
				? pathUris.push(
						vscode.Uri.file(
							uriPath.concat(Constants.WebFileYmlExtension),
						),
					)
				: pathUris.push(
						vscode.Uri.file(
							uriPath.substring(0, ymlExtensionIndex),
						),
					);
		} else if (!isSingleFileEntity(fileEntityType)) {
			const folderPathNameIndex = getEntityFolderPathIndex(
				uriPath,
			...
			pathUris.push(
				vscode.Uri.file(uriPath.substring(0, folderPathNameIndex)),
			);
		}
	}
...
export function getEntityFolderPathIndex(
	uriPath: string,
	fileName: string,
	fileEntityType: Constants.PowerPagesEntityType,
	entityFolderName: string,
) {
	return isSingleFileEntity(fileEntityType)
		? uriPath.indexOf(`/${entityFolderName}/`) + entityFolderName.length + 2
		: uriPath.indexOf(`/${fileName?.toLowerCase()}/`) +
				fileName?.length +
				2; // offset for path separator
}

export function getValidatedEntityPath(
	folderPath: string,
	fileName: string,
	fileExtension: string,
): vscode.Uri {
	return vscode.Uri.file(
		path.join(folderPath, [fileName, fileExtension].join(".")),
	);
}

export function isValidRenamedFile(
	uriPath: string,
	entityFolderName: string,
	fileName: string,
	fileEntityType: Constants.PowerPagesEntityType,
	...
	return isSingleFileEntity(fileEntityType)
		? uriPath.includes(`/${entityFolderName}/${fileName}`)
		: uriPath.includes(`/${entityFolderName}/${fileName.toLowerCase()}/`);
}

export function getUpdatedFolderPath(
	uriPath: string,
	oldFileName: string,
	newFileName: string,
): vscode.Uri {
...
export function isSingleFileEntity(
	fileEntityType: Constants.PowerPagesEntityType,
) {
	return (
		fileEntityType === Constants.PowerPagesEntityType.WEBFILES ||
...
export function getFileNameProperties(
	uriPath: string,
	fileEntityType: Constants.PowerPagesEntityType,
): IFileNameProperties {
	...
	if (fileProperties.fileName) {
		const ymlExtensionIndex =
			fileProperties.fileCompleteName?.indexOf(
				Constants.WebFileYmlExtension,
			) ?? -1;
		...
		if (
			...
			formattedName = fileProperties.fileCompleteName?.substring(
				0,
				ymlExtensionIndex,
			);
		}
...
export function getExcludedFileGlobPattern(
