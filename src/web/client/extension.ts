...
import type { IPortalWebExtensionInitQueryParametersTelemetryData } from "../../common/OneDSLoggerTelemetry/web/client/webExtensionTelemetryInterface";
...
export function createWebWorkerInstance(context: vscode.ExtensionContext) {
	try {
		...
		WebExtensionContext.telemetry.sendInfoTelemetry(
			webExtensionTelemetryEventNames.WEB_EXTENSION_WEB_WORKER_REGISTERED,
		);
	} catch (error) {
		WebExtensionContext.telemetry.sendErrorTelemetry(
...
function showNotificationForCopilot(
	...
	if (!isCopilotNotificationDisabled) {
		WebExtensionContext.telemetry.sendInfoTelemetry(
			webExtensionTelemetryEventNames.WEB_EXTENSION_WEB_COPILOT_NOTIFICATION_SHOWN,
			{ orgId: orgId },
		);
...
async function fetchArtemisData(orgId: string) {
	...
	WebExtensionContext.geoLongName =
		artemisResponse?.response?.geoLongName ?? "";
	WebExtensionContext.serviceEndpointCategory =
		artemisResponse?.stamp ?? ServiceEndpointCategory.NONE;
}
