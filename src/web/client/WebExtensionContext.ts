...
import type { IAttributePath, IEntityInfo } from "./common/interfaces";
...
import { type IConnectionData, UserDataMap } from "./context/userDataMap";
import { ConcurrencyHandler } from "./dal/concurrencyHandler";
import { type SchemaEntityMetadata, schemaKey } from "./schema/constants";
import {
...
class WebExtensionContext implements IWebExtensionContext {
	...
	public setWebExtensionContext(
		...
		this._defaultEntityType =
			...
			(queryParamsMap.get(Constants.queryParameters.ENTITY) as string) ??
			"";
		this._defaultEntityId =
			entityId ??
			(queryParamsMap.get(
				Constants.queryParameters.ENTITY_ID,
			) as string) ??
			"";
	...
	public setVscodeWorkspaceState(workspaceState: vscode.Memento) {
		try {
			...
			this.telemetry.sendInfoTelemetry(
				webExtensionTelemetryEventNames.WEB_EXTENSION_SET_VSCODE_WORKSPACE_STATE_SUCCESS,
	...
	public async authenticateAndUpdateDataverseProperties() {
		...
		const headers = getCommonHeadersForDataverse(
			this._dataverseAccessToken,
		);
	...
	public async getWorkerScript(workerUrl: URL): Promise<any> {
		try {
			...
			const response =
				await this.concurrencyHandler.handleRequest(workerUrl);
	...
	public async removeConnectedUserInContext(
		userId: string,
		removeConnectionData: IConnectionData,
	) {
