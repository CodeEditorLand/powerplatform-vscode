...
import { MIMETYPE, httpMethod, queryParameters } from "../common/constants";
...
// TODO - Make Json for different response type and update any here
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function getMappingEntityContent(
	entity: string,
	result: any,
	attribute: string,
) {
	const mappedEntity = getEntity(entity)?.get(schemaEntityKey.MAPPING_ENTITY);
...
export function isValidDirectoryPath(fsPath: string): boolean {
	return (
		WebExtensionContext.isContextSet &&
		fsPath.toLowerCase() ===
			WebExtensionContext.rootDirectory.fsPath.toLowerCase()
	);
}

export function isWebFileWithLazyLoad(fsPath: string): boolean {
	const isPreloadedContent =
		WebExtensionContext.fileDataMap.getFileMap.get(fsPath)?.isContentLoaded;
