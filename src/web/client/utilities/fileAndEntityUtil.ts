...
import type { SchemaEntityMetadata } from "../schema/constants";
...
export function getEntityMetadata(fileFsPath: string) {
	return WebExtensionContext.fileDataMap.getFileMap.get(fileFsPath)
		?.entityMetadata as SchemaEntityMetadata;
}
...
export function doesFileExist(fileFsPath: string) {
	return WebExtensionContext.fileDataMap.getFileMap.has(
		vscode.Uri.parse(fileFsPath).fsPath,
	);
}
...
export function updateEntityEtag(entityId: string, entityEtag: string) {
	WebExtensionContext.entityDataMap.updateEtagValue(entityId, entityEtag);
}
