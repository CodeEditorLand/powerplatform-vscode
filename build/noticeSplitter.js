...
const noticeFileFQN =
	process.argv[2] ||
	path.resolve(os.userInfo().homedir, "Downloads", "NOTICE.txt");
const summaryFile = path.resolve(repoRoot, "out", "noticeSplitterResults.txt");

const nugetNoticeFiles = [].map((fileName) => path.resolve(repoRoot, fileName));

const npmNoticeFiles = ["./NOTICE.txt"].map((fileName) =>
...
class Splitter {
	...
	determineCompType(line) {
		...
		if (line.length > 0) {
			...
			const compType =
				firstChar.toLowerCase() === firstChar || firstChar === "@"
					? CompTypeEnum.Npm
					: CompTypeEnum.Nuget;
	...
	createNoticeFiles() {
		...
		this.#npmNotices = this.#npmNotices.sort((a, b) =>
			a.title
				.toLocaleLowerCase()
		...
		npmNoticeFiles.forEach((fileName) => {
			console.log(
				`writing npm notice file  : ${path.relative(p.cwd(), fileName)}`,
			);
			fs.writeFileSync(fileName, npmNotice, { encoding: "utf8" });
		});

		this.#nugetNotices = this.#nugetNotices.sort((a, b) =>
			a.title
				.toLocaleLowerCase()
				.localeCompare(b.title.toLocaleLowerCase()),
		);
		...
		nugetNoticeFiles.forEach((fileName) => {
			console.log(
				`writing nuget notice file: ${path.relative(p.cwd(), fileName)}`,
			);
