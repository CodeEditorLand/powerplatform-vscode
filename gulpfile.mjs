...
async function nugetInstall(nugetSource, packageName, version, targetDir) {
	...
	const feeds = {
		...
		"Power_Platform_Tools_Feed": {
			authenticated: true,
			baseUrl:
				"https://pkgs.dev.azure.com/dynamicscrm/OneCRM/_packaging/6f6505ca-e632-4c4c-9115-6d45f61758cc/nuget/v3/flat2/",
		},
	};
	...
	if (!res.ok) {
		...
		throw new Error(
			`Cannot download ${res.url}, status: ${res.statusText} (${res.status}), body: ${body ? body.toString("ascii") : "<empty>"}`,
		);
	}
