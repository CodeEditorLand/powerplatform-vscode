...
export class CliAcquisition implements IDisposable {
			...
			return new Promise((resolve, reject) => {
				...
				if (install.status != 0) {
					...
					const dotnetIncompatible =
						install.stdout.includes("NU1202") ||
						install.stderr.includes("NU1202");
	...
	async findPacProcesses(
		pacInstallPath: string,
		...
		try {
			...
			const processes = (await find("name", "pac", true)).concat(
				await find("name", "pacTelemetryUpload", false),
			); // strict = false, as this may either be 'pacTelemetryProcess' or 'dotnet pacTelemetryProcess.dll'
			...
			return processes.filter((info) =>
				os.platform() === "win32"
					? info.cmd
							.toLowerCase()
							.includes(pacInstallPath.toLowerCase())
			...
			if (
				typeof err === "string" &&
				err.includes("screen size is bogus") &&
				os.platform() === "linux"
			) {
	...
	getLatestNupkgVersion(): string {
		...
		const versions = glob
			.sync(`${basename}*${nuPkgExtension}`, { cwd: this._nupkgsFolder })
			.map((file) =>
				file
					.substring(basename.length + 1)
					.slice(0, -nuPkgExtension.length),
