		...
		vscode.Disposable
{
	...
	constructor(
		...
		public readonly solutionDataSource: (
	...
	public async getChildren(
		...
		if (!element) {
			...
			if (
				...
				return envOutput.Results.map(
					(item) => new EnvOrSolutionTreeItem(item),
				);
			} else {
				return [];
			}
			...
			if (
				...
				solutionOutput.Results
			) {
				return solutionOutput.Results.map(
					(item) => new EnvOrSolutionTreeItem(item),
				);
			} else {
				return [];
	...
	private registerPanel(pacWrapper: PacWrapper): vscode.Disposable[] {
		return [
			vscode.window.registerTreeDataProvider(
				"pacCLI.envAndSolutionsPanel",
				this,
			),
			vscode.commands.registerCommand(
				"pacCLI.envAndSolutionsPanel.refresh",
				() => this.refresh(),
			),
			vscode.commands.registerCommand(
				"pacCLI.envAndSolutionsPanel.copyDisplayName",
				(item: EnvOrSolutionTreeItem) => {
					vscode.env.clipboard.writeText(
						(item.model as OrgListOutput).FriendlyName,
					);
				},
			),
			vscode.commands.registerCommand(
				"pacCLI.envAndSolutionsPanel.copyEnvironmentId",
				(item: EnvOrSolutionTreeItem) => {
					vscode.env.clipboard.writeText(
						(item.model as OrgListOutput).EnvironmentId,
					);
				},
			),
			vscode.commands.registerCommand(
				"pacCLI.envAndSolutionsPanel.copyEnvironmentUrl",
				(item: EnvOrSolutionTreeItem) => {
					vscode.env.clipboard.writeText(
			...
			vscode.commands.registerCommand(
				"pacCLI.envAndSolutionsPanel.selectEnvironment",
				async (item: EnvOrSolutionTreeItem) => {
			...
			vscode.commands.registerCommand(
				"pacCLI.envAndSolutionsPanel.copyOrganizationId",
				(item: EnvOrSolutionTreeItem) => {
					vscode.env.clipboard.writeText(
						(item.model as OrgListOutput).OrganizationId,
					);
				},
			),
			vscode.commands.registerCommand(
				"pacCLI.envAndSolutionsPanel.copyFriendlyName",
				(item: EnvOrSolutionTreeItem) => {
					vscode.env.clipboard.writeText(
						(item.model as SolutionListing).FriendlyName,
					);
				},
			),
			vscode.commands.registerCommand(
				"pacCLI.envAndSolutionsPanel.copyUniqueName",
				(item: EnvOrSolutionTreeItem) => {
					vscode.env.clipboard.writeText(
						(item.model as SolutionListing).SolutionUniqueName,
					);
				},
			),
			vscode.commands.registerCommand(
				"pacCLI.envAndSolutionsPanel.copyVersionNumber",
				(item: EnvOrSolutionTreeItem) => {
					vscode.env.clipboard.writeText(
						(item.model as SolutionListing).VersionNumber,
					);
				},
			),
		];
...
class EnvOrSolutionTreeItem extends vscode.TreeItem {
	...
	private static setCollapsibleState(
		model: OrgListOutput | SolutionListing,
