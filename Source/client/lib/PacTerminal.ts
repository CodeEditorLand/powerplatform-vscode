...
export class PacTerminal implements vscode.Disposable {
	...
	public constructor(
		...
		this._context.environmentVariableCollection.prepend(
			"PATH",
			cliPath + path.delimiter,
		);
		...
		if (os.platform() === "darwin" && os.version().includes("ARM64")) {
			this._context.environmentVariableCollection.replace(
				"DOTNET_ROLL_FORWARD",
				"Major",
			);
		}
		...
		this._cmdDisposables.push(
			vscode.commands.registerCommand("pacCLI.pacHelp", () =>
				PacTerminal.getTerminal().sendText("pac help"),
			),
			vscode.commands.registerCommand("pacCLI.pacAuthHelp", () =>
				PacTerminal.getTerminal().sendText("pac auth help"),
			),
			vscode.commands.registerCommand("pacCLI.pacPackageHelp", () =>
				PacTerminal.getTerminal().sendText("pac package help"),
			),
			vscode.commands.registerCommand("pacCLI.pacPcfHelp", () =>
				PacTerminal.getTerminal().sendText("pac pcf help"),
			),
			vscode.commands.registerCommand("pacCLI.pacSolutionHelp", () =>
				...
				async () => {
					...
					if (result?.Status === "Success") {
						...
						vscode.window.showErrorMessage(
							vscode.l10n.t("Failed to enable PAC telemetry."),
						);
					}
				},
			),
			...
			vscode.commands.registerCommand(
				...
				async () => {
					...
					if (result?.Status === "Success") {
						...
						vscode.window.showErrorMessage(
							vscode.l10n.t("Failed to disable PAC telemetry."),
						);
					}
				},
			),
	...
	public openDocumentation(): void {
		vscode.env.openExternal(
			vscode.Uri.parse("https://aka.ms/pacvscodedocs"),
		);
	}

	public openPacLab(): void {
		vscode.env.openExternal(
			vscode.Uri.parse("https://aka.ms/powerplatform-vscode-lab"),
		);
	}
