...
import type { AuthProfileListing, PacAuthListOutput } from "../pac/PacTypes";
	...
	implements vscode.TreeDataProvider<AuthProfileTreeItem>, vscode.Disposable
{
	...
	public async getChildren(
		...
		if (element) {
			...
			if (
				...
				const items = pacOutput.Results.filter(
	...
	private registerPanel(pacWrapper: PacWrapper): vscode.Disposable[] {
		return [
					...
					const confirmResult =
						await vscode.window.showWarningMessage(
							vscode.l10n.t({
								...
								args: [
									item.model.UserDisplayName,
									item.model.ActiveOrganization?.Item2,
								],
								comment: [
			...
			vscode.commands.registerCommand(
				"pacCLI.authPanel.navigateToResource",
				(item: AuthProfileTreeItem) => {
...
class AuthProfileTreeItem extends vscode.TreeItem {
	...
	private static createLabel(profile: AuthProfileListing): string {
		if (profile.Name) {
			...
			return `${profile.Kind}: ${
				profile.ActiveOrganization?.Item2 ?? profile.UserDisplayName
			}`;
		}
	}
...
export function GetAuthProfileWatchPattern():
