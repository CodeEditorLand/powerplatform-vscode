...
import {
	CliAcquisition,
	type ICliAcquisitionContext,
} from "./lib/CliAcquisition";
...
import {
	getPortalsOrgURLs,
	workspaceContainsPortalConfigFolder,
} from "../common/PortalConfigFinder";
...
import type { ActiveOrgOutput } from "./pac/PacTypes";
...
export async function activate(
	...
	_context.subscriptions.push(
		vscode.commands.registerCommand(
			"microsoft-powerapps-portals.bootstrap-diff",
			async () => {
				_telemetry.sendTelemetryEvent("StartCommand", {
		...
		orgChangeEvent(async (orgDetails: ActiveOrgOutput) => {
			...
			if (artemisResponse !== null && artemisResponse.length > 0) {
				const { geoName, geoLongName } = artemisResponse[0]
					?.response as unknown as IArtemisAPIOrgResponse;
				oneDSLoggerWrapper.instantiate(geoName, geoLongName);
				oneDSLoggerWrapper
					.getLogger()
					.traceInfo(
						telemetryEventNames.DESKTOP_EXTENSION_INIT_CONTEXT,
						{ ...orgDetails, orgGeo: geoName },
					);
			}
		}),
	);
	...
	if (workspaceContainsPortalConfigFolder(workspaceFolders)) {
		...
		try {
			...
			_telemetry.sendTelemetryEvent("VscodeDesktopUsage", {
				...
				countOfActivePortals: listOfActivePortals.length.toString(),
			});
			oneDSLoggerWrapper.getLogger().traceInfo("VscodeDesktopUsage", {
				listOfActivePortals: telemetryData,
				countOfActivePortals: listOfActivePortals.length.toString(),
			});
		} catch (exception) {
		...
		OrgChangeNotifier.createOrgChangeNotifierInstance(
			pacTerminal.getWrapper(),
		);
...
function showNotificationForCopilot(
	...
	countOfActivePortals: string,
) {
	if (
		vscode.workspace
			.getConfiguration("powerPlatform")
			.get("experimental.copilotEnabled") === false
	) {
		return;
	}

	const isCopilotNotificationDisabled = _context.globalState.get(
...
class CliAcquisitionContext implements ICliAcquisitionContext {
	...
	showCliReadyMessage(): void {
		vscode.window.showInformationMessage(
			vscode.l10n.t(
				"The pac CLI is ready for use in your VS Code terminal!",
			),
		);
	}
