...
import {
	...
	EnvIdKey,
	TenantIdKey,
} from "../common/OneDSLoggerTelemetry/telemetryConstants";
...
import { orgChangeEvent, OrgChangeNotifier } from "./OrgChangeNotifier";
		...
		orgChangeEvent(async (orgDetails: ActiveOrgOutput) => {
			...
			if (artemisResponse !== null && artemisResponse.response !== null) {
				...
				if (
					...
					await ECSFeaturesClient.init(
						_telemetry,
						{
							...
							Location: getECSOrgLocationValue(
								clusterName,
								clusterNumber,
							),
						},
				...
				oneDSLoggerWrapper
					.getLogger()
					.traceInfo(
						desktopTelemetryEventNames.DESKTOP_EXTENSION_INIT_CONTEXT,
						initContext,
					);
			}
