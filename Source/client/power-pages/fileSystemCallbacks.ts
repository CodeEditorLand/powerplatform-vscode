		...
		vscode.workspace.onDidDeleteFiles(async (e) => {
			...
			if (e.files.length > 0) {
				...
				try {
					...
					await Promise.all(
						e.files.map(async (f) => {
							...
							sendTelemetryEvent(telemetry, {
								...
								fileEntityType:
									PowerPagesEntityType[fileEntityType],
								methodName: processOnDidDeleteFiles.name,
							});

							if (
								fileEntityType === PowerPagesEntityType.UNKNOWN
							) {
								return;
							}
					...
					sendTelemetryEvent(telemetry, {
						methodName: processOnDidDeleteFiles.name,
						eventName: UserFileDeleteEvent,
						numberOfFiles: e.files.length.toString(),
				...
				try {
					...
					await Promise.all(
						e.files.map(async (f) => {
							...
							sendTelemetryEvent(telemetry, {
								methodName: processOnDidRenameFiles.name,
								eventName: FileRenameEvent,
								fileEntityType:
									PowerPagesEntityType[fileEntityType],
							});
