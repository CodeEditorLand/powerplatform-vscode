...
import { ValidateTextDocumentEvent, sendTelemetryEvent } from "./telemetry";

// Create a diagnostics connection to output warning/error messages to "Problems" tab
const connection =
	vscode.languages.createDiagnosticCollection("FileDeleteEvent");
...
export async function validateTextDocument(
	...
	try {
				...
				const diagnostic: vscode.Diagnostic = {
					...
					message:
						...
						(searchByName
							? vscode.l10n.t({
									message:
										"File might be referenced by name {0} here.",
									args: [m[0]],
									comment: [
		...
		connection.set(
			uri,
			diagnostics.concat(vscode.languages.getDiagnostics(uri)),
		);
	} catch (error) {
		sendTelemetryEvent(telemetry, {
			methodName: validateTextDocument.name,
			eventName: ValidateTextDocumentEvent,
