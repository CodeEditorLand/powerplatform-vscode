...
import {
	CleanupRelatedFilesEvent,
	FileRenameValidationEvent,
	sendTelemetryEvent,
	UpdateEntityNameInYmlEvent,
	UpdateEntityPathNamesEvent,
...
export async function fileRenameValidation(
	oldUri: vscode.Uri,
	newUri: vscode.Uri,
	...
	try {
		if (isValidUri(oldUri.path) && oldFileProperties.fileName) {
			...
			if (
				oldFileProperties.fileExtension !==
					newFileProperties.fileExtension &&
				newFileProperties.fileName &&
				oldFileProperties.fileExtension
			) {
				await vscode.workspace.fs.rename(
					newUri,
					getValidatedEntityPath(
						newFileProperties.fileFolderPath,
						newFileProperties.fileName,
						oldFileProperties.fileExtension,
					),
					{ overwrite: true },
				);
			} else if (oldFileProperties.fileExtension === "") {
				// revert name changes - entity folder re-naming is not a valid scenario
		...
		sendTelemetryEvent(telemetry, {
			methodName: fileRenameValidation.name,
			eventName: FileRenameValidationEvent,
...
export async function updateEntityPathNames(
	oldUri: vscode.Uri,
	newUri: vscode.Uri,
	oldFileProperties: IFileProperties,
	...
	try {
		...
		if (
			...
			const newFileProperties = getFileProperties(newUri.path);
			const entityFolderIndex = getEntityFolderPathIndex(
				newUri.path,
				oldFileProperties.fileName,
				fileEntityType,
				entityFolderName,
			);
			const fileEntityFolderPath =
				oldFileProperties.fileFolderPath.substring(
					0,
					entityFolderIndex,
				);
			const allFilesInFolder = await vscode.workspace.findFiles(
				fileEntityType === PowerPagesEntityType.WEBFILES
					? `**/${entityFolderName}/${oldFileProperties.fileName}.*`
					: `**/${entityFolderName}/${oldFileProperties.fileName.toLowerCase()}/**/*`,
			);
			...
			if (newFileProperties.fileName) {
				if (fileEntityType !== PowerPagesEntityType.ADVANCED_FORMS) {
					for (const file of allFilesInFolder) {
						...
						if (fileProperties.fileCompleteName) {
							const newFilePath = file.fsPath.replace(
								fileProperties.fileCompleteName,
								[
									newFileProperties.fileName,
									fileProperties.fileExtension,
								].join("."),
							);
				...
				const ymlFileInFolder = await vscode.workspace.findFiles(
					isUpdateNeeded
				...
				if (!isUpdateNeeded) {
					await vscode.workspace.fs.rename(
						vscode.Uri.file(
							removeTrailingSlash(fileEntityFolderPath),
						),
						getUpdatedFolderPath(
							fileEntityFolderPath,
							oldFileProperties.fileName,
							newFileProperties.fileName,
						),
						{ overwrite: true },
		...
		sendTelemetryEvent(telemetry, {
			methodName: updateEntityPathNames.name,
			eventName: UpdateEntityPathNamesEvent,
			exception: e as Error,
		});
	}
}

export async function cleanupRelatedFiles(
	uriPath: string,
	fileEntityType: PowerPagesEntityType,
	...
	try {
		const pathUris = getDeletePathUris(
			uriPath,
			fileEntityType,
			fileProperties,
		);
		...
		sendTelemetryEvent(telemetry, {
			methodName: cleanupRelatedFiles.name,
			eventName: CleanupRelatedFilesEvent,
			exception: e as Error,
		});
	}
}

function updateEntityNameInYml(
	uriPath: string,
	fileEntityType: PowerPagesEntityType,
	telemetry: ITelemetry,
) {
	try {
		...
		const fileNameProperties = getFileNameProperties(
			uriPath,
			fileEntityType,
		);
		...
		fieldsToUpdate.forEach((field) => {
			if (field === DataverseFieldAdxPartialUrl) {
				...
				parsedFileContents[field] =
					fileNameProperties.formattedFileName;
			}
		});
		...
		sendTelemetryEvent(telemetry, {
			methodName: updateEntityNameInYml.name,
			eventName: UpdateEntityNameInYmlEvent,
			exception: e as Error,
		});
	}
}
