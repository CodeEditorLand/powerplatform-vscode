...
import { TokenKind, Tokenizer } from "liquidjs";
import type { OutputToken, TagToken } from "liquidjs/dist/tokens";
...
import type { IAutoCompleteTelemetryData } from "../../common/TelemetryData";
...
import {
	type ILiquidAutoCompleteRule,
	ruleDefinitions,
} from "./LiquidAutoCompleteRule";
...
const getSuggestionsFromRules = (
	liquidToken: TagToken | OutputToken,
	context: ILiquidRuleEngineContext,
): CompletionItem[] => {
	return rules
		.sort((r1, r2) => {
...
export const getSuggestions = (
	...
	pathOfFileBeingEdited: string,
	workspaceRootFolders: WorkspaceFolder[] | null,
	editedTextDocument: TextDocument,
	connection: Connection,
) => {
	...
	try {
		return (
			getLiquidExpression(
				editedLine,
				colIndex,
				liquidTagStartExpression,
				liquidTagEndExpression,
			) ||
			getLiquidExpression(
				editedLine,
				colIndex,
				liquidOutputStartExpression,
				liquidOutputEndExpression,
			)
		);
	} catch (e) {
...
const getLiquidExpression = (
	...
	if (
		...
		return {
			LiquidExpression:
				editedLiquidExpressionOnLeftOfCursor +
				editedLiquidExpressionOnRightOfCursor,
