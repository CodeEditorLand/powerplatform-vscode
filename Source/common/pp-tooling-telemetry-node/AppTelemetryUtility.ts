...
import type { IAppTelemetryEnvironment } from "./interfaces";
...
export function isSupportedAgentProductVersion(
	productVersion: string,
): boolean {
	if (productVersion) {
...
export function tryParseAutomationAgentStringProductNameVersion(agent: string):
	...
	if (!productName || !ProductNameRegex.test(productName)) {
		return {
			success: false,
			invalidReason:
				"Product name is empty or contains characters other than alpha-numeric and '.', '+', '_', '-'.",
		};
	}
	...
	if (!isSupportedAgentProductVersion(productVersion)) {
		return {
			success: false,
			invalidReason:
				"Product version is empty or is not a valid System.Version or SemVer (v2).",
		};
	}
...
function validateAndSanitizeAutomationAgent(
	...
	if (result.success !== true) {
		...
		throw new Error(
			`[AppTelemetryConfigurationException] The automationAgent '${environment.automationAgent}' is invalid. ${result.invalidReason} This exception should be considered a developer bug of the calling agent.`,
		);
	}
	...
	if (agent.length > MaxAutomationAgentLength) {
		...
		agent =
			composeAutomationAgentString(
				result.firstProductName,
				result.firstProductVersion,
			) + " --truncated--";
	}
...
export function createCommonAppStartProperties(
	environment: IAppTelemetryEnvironment,
