...
export async function getIntelligenceEndpoint(
	orgId: string,
	telemetry: ITelemetry,
	sessionID: string,
) {
	const artemisResponse = await fetchArtemisResponse(
		orgId,
		telemetry,
		sessionID,
	);
	...
	const intelligenceEndpoint = `https://aibuildertextapiservice.${geoName}-${
		"il" + clusterNumber
	}.gateway.${environment}.island.powerapps.com/v1.0/${orgId}/appintelligence/chat`;

	return { intelligenceEndpoint: intelligenceEndpoint, geoName: geoName };
}

// Function to fetch Artemis response
export async function fetchArtemisResponse(
	orgId: string,
	telemetry: ITelemetry,
	sessionID = "",
) {
	...
	const artemisResponse = await fetchIslandInfo(
		endpoints,
		telemetry,
		sessionID,
	);
...
async function fetchIslandInfo(
	endpoints: string[],
	telemetry: ITelemetry,
	sessionID: string,
) {
