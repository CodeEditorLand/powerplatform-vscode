...
import type { IContextInfo, IUserInfo } from "./IEventTypes";
import type { ITelemetryLogger } from "./ITelemetryLogger";
...
export class OneDSLogger implements ITelemetryLogger {
	...
	private static userRegion = "";

	private readonly regexPatternsToRedact = [
		/key["\\ ']*[:=]+["\\ ']*([a-zA-Z0-9]*)/gim,
		/token["\\ ']*[:=]+["\\ ']*([a-zA-Z0-9]*)/gim,
		/session["\\ ']*[:=]+["\\ ']*([a-zA-Z0-9]*)/gim,
	];
	...
	public constructor(geo?: string, geoLongName?: string) {
		...
		const instrumentationSetting: IInstrumentationSettings =
			OneDSLogger.getInstrumentationSettings(geo, geoLongName); // Need to replace with actual data
	...
	private static getInstrumentationSettings(
		geo?: string,
		geoLongName?: string,
	): IInstrumentationSettings {
		...
		const instrumentationSettings: IInstrumentationSettings = {
			...
			instrumentationKey:
				"ffdb4c99ca3a4ad5b8e9ffb08bf7da0d-65357ff3-efcd-47fc-b2fd-ad95a52373f4-7402",
		};
		switch (geoLongName) {
			case "usgov":
				geo = "gov";
				...
				switch (geo) {
					...
					case "kr":
						(instrumentationSettings.endpointURL =
							...
							(instrumentationSettings.instrumentationKey =
					...
					case "gov":
						(instrumentationSettings.endpointURL =
							...
							(instrumentationSettings.instrumentationKey =
								"2f217cb8f40440eeb8b0aa80a2be2f7e-e0ec7b51-d1bb-4d8c-83b1-cc77aaba9009-7472");
						break;
					case "high":
						(instrumentationSettings.endpointURL =
							"https://tb.events.data.microsoft.com/OneCollector/1.0/"),
							(instrumentationSettings.instrumentationKey =
								"4a07e143372c46aabf3841dc4f0ef795-a753031e-2005-4282-9451-a086fea4234a-6942");
						break;
					case "dod":
						(instrumentationSettings.endpointURL =
							"https://pf.events.data.microsoft.com/OneCollector/1.0/"),
							(instrumentationSettings.instrumentationKey =
								"af47f3d608774379a53fa07cf36362ea-69701588-1aad-43ee-8b52-f71125849774-6656");
						break;
					case "mooncake":
						(instrumentationSettings.endpointURL =
							"https://collector.azure.cn/OneCollector/1.0/"),
							(instrumentationSettings.instrumentationKey =
								"f9b6e63b5e394453ba8f58f7a7b9aea7-f38fcfa2-eb34-48bc-9ae2-61fba4abbd39-7390"); //prod key;
						break;
					default:
						(instrumentationSettings.endpointURL =
							"https://us-mobile.events.data.microsoft.com/OneCollector/1.0/"),
	...
	private populateVscodeWebAttributes(envelope: any) {
		...
		if (
			envelope.data.eventName == telemetryEventNames.WEB_EXTENSION_ORG_GEO
		) {
	...
	private redactSensitiveDataFromEvent(envelope: any) {
		...
		envelope.data.errorStack = this.getAllSensitiveRedactedFromField(
			envelope.data.errorStack,
		);
		return envelope;
	}
	...
	getRedactedValueViaRegexMatch(value: string, regexPattern: RegExp) {
		...
		while ((matches = regexPattern.exec(value)) !== null) {
			...
			matches.forEach((match, groupIndex) => {
				if (groupIndex == 0) {
					// Redact the entire matched string
					value = value.replace(
						match,
						OneDSCollectorEventName.REDACTED,
					); //Replace with string REDACTED
				}
			});
		}
		return value;
