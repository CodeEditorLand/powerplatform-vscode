...
import {
	handleOrgChangeSuccess,
	initializeOrgDetails,
	ORG_DETAILS_KEY,
} from "../../utilities/OrgHandlerUtils";
import {
...
import {
	...
	registerCommands,
} from "../ChatParticipantUtils";
import { CommandRegistry } from "../CommandRegistry";
import {
	AUTHENTICATION_FAILED_MSG,
	COPILOT_NOT_AVAILABLE_MSG,
	COPILOT_NOT_RELEASED_MSG,
	DISCLAIMER_MESSAGE,
	INVALID_RESPONSE,
	NO_PROMPT_MESSAGE,
	PAC_AUTH_INPUT,
	PAC_AUTH_NOT_FOUND,
	POWERPAGES_CHAT_PARTICIPANT_ID,
	POWERPAGES_COMMANDS,
	RESPONSE_AWAITED_MSG,
	RESPONSE_SCENARIOS,
	SKIP_CODES,
	STATER_PROMPTS,
	WELCOME_MESSAGE,
...
import {
	...
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_ERROR,
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_INVOKED,
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_LOCATION_REFERENCED,
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_NO_PROMPT,
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_NOT_AVAILABLE_ECS,
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_ORG_DETAILS,
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_ORG_DETAILS_NOT_FOUND,
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_SCENARIO,
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_SUCCESSFUL_PROMPT,
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_WEBPAGE_RELATED_FILES,
	VSCODE_EXTENSION_GITHUB_POWER_PAGES_AGENT_WELCOME_PROMPT,
} from "./PowerPagesChatParticipantTelemetryConstants";
import {
	IComponentInfo,
	IPowerPagesChatResult,
} from "./PowerPagesChatParticipantTypes";
import {
	createAndReferenceLocation,
	createErrorResult,
	createSuccessResult,
	getComponentInfo,
	getEndpoint,
	handleChatParticipantFeedback,
	provideChatParticipantFollowups,
	removeChatVariables,
...
export class PowerPagesChatParticipant {
	...
	private constructor(
		...
		this.chatParticipant = createChatParticipant(
			POWERPAGES_CHAT_PARTICIPANT_ID,
			this.handler,
		);

		//TODO: Check the icon image
		this.chatParticipant.iconPath = vscode.Uri.joinPath(
		...
		this.chatParticipant.onDidReceiveFeedback(
		...
		this._disposables.push(
			orgChangeErrorEvent(async () => {
				this.extensionContext.globalState.update(ORG_DETAILS_KEY, {
					orgID: undefined,
					orgUrl: undefined,
				});
			}),
	...
	public static getInstance(
		...
		if (!PowerPagesChatParticipant.instance) {
			PowerPagesChatParticipant.instance = new PowerPagesChatParticipant(
				context,
				telemetry,
				pacWrapper,
			);
		...
		try {
			...
			if (!this.orgID || !this.environmentID) {
				...
				return createErrorResult(
					PAC_AUTH_NOT_FOUND,
					RESPONSE_SCENARIOS.PAC_AUTH_NOT_FOUND,
					"",
				);
			}
			...
			if (!isPowerPagesGitHubCopilotEnabled()) {
				...
				return createSuccessResult(
					"",
					RESPONSE_SCENARIOS.COPILOT_NOT_RELEASED,
					this.orgID,
				);
			}

			const intelligenceApiAuthResponse =
				await intelligenceAPIAuthentication(
					this.telemetry,
					this.powerPagesAgentSessionId,
					this.orgID,
					true,
			...
			if (!intelligenceApiAuthResponse) {
				return createErrorResult(
					AUTHENTICATION_FAILED_MSG,
					RESPONSE_SCENARIOS.AUTHENTICATION_FAILED,
					this.orgID,
				);
			}

			const intelligenceApiToken =
				intelligenceApiAuthResponse.accessToken;
			const userId = intelligenceApiAuthResponse.userId;
			const intelligenceAPIEndpointInfo = await getEndpoint(
				this.orgID,
				this.environmentID,
				this.telemetry,
				this.cachedEndpoint,
			...
			if (
				!copilotAvailabilityStatus ||
				!intelligenceAPIEndpointInfo.intelligenceEndpoint
			) {
				return createErrorResult(
					COPILOT_NOT_AVAILABLE_MSG,
					RESPONSE_SCENARIOS.COPILOT_NOT_AVAILABLE,
					this.orgID,
				);
			}
			...
			if (userPrompt === WELCOME_PROMPT) {
				...
				return createSuccessResult(
					STATER_PROMPTS,
					RESPONSE_SCENARIOS.WELCOME_PROMPT,
					this.orgID,
				);
			} else if (!userPrompt) {
			...
			const location = activeFileUri
				? createAndReferenceLocation(activeFileUri, startLine, endLine)
				: undefined;
			...
			if (request.command) {
				...
				switch (activeFileParams.dataverseEntity) {
					case ADX_WEBPAGE:
						if (activeFileUri) {
							...
							const files = await fetchRelatedFiles(
								activeFileUri,
								activeFileParams.dataverseEntity,
								activeFileParams.fieldType,
								this.telemetry,
								this.powerPagesAgentSessionId,
							);
				...
				const { componentInfo, entityName }: IComponentInfo =
				...
				const scenario =
					llmResponse.length > 1
						? llmResponse[llmResponse.length - 1]
						: llmResponse[0].displayText;
	...
	private async initializeOrgDetails(): Promise<void> {
		try {
			const { orgID, orgUrl, environmentID } = await initializeOrgDetails(
				this.isOrgDetailsInitialized,
				this._pacWrapper,
			);
	...
	private async handleOrgChangeSuccess(
