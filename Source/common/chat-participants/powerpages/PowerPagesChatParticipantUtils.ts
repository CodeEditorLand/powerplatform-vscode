...
import {
	...
	getEntityName,
	getFormXml,
} from "../../copilot/dataverseMetadata";
import type { IActiveFileParams } from "../../copilot/model";
...
export async function getEndpoint(
	...
	if (!cachedEndpoint) {
		cachedEndpoint = (await ArtemisService.getIntelligenceEndpoint(
			orgID,
			telemetry,
			sessionID,
			environmentID,
...
export async function getComponentInfo(
	...
	if (isEntityInSupportedList(activeFileParams.dataverseEntity)) {
		...
		const dataverseToken = (
			await dataverseAuthentication(telemetry, orgUrl ?? "", true)
		).accessToken;
...
export function createAndReferenceLocation(
...
export function provideChatParticipantFollowups(
	result: IPowerPagesChatResult,
