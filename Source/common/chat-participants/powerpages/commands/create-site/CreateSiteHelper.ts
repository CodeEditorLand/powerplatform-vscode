...
import {
	NL2PAGE_GENERATING_WEBPAGES,
	NL2PAGE_RESPONSE_FAILED,
	NL2SITE_REQUEST_FAILED,
} from "../../PowerPagesChatParticipantConstants";
import {
	VSCODE_EXTENSION_NL2PAGE_REQUEST,
	VSCODE_EXTENSION_NL2SITE_REQUEST,
...
import {
	ICreateSiteOptions,
	IPreviewSitePagesContentOptions,
} from "./CreateSiteTypes";
...
export const createSite = async (createSiteOptions: ICreateSiteOptions) => {
	...
	const { siteName, siteDescription, sitePages } = await fetchSiteAndPageData(
		intelligenceEndpoint,
		intelligenceApiToken,
		userPrompt,
		sessionId,
	...
	const contentProvider = previewSitePagesContent({
		sitePages,
		stream,
		extensionContext,
		telemetry,
...
async function fetchSiteAndPageData(
	...
	telemetry.sendTelemetryEvent(VSCODE_EXTENSION_NL2SITE_REQUEST, {
		sessionId: sessionId,
		orgId: orgId,
		environmentId: envId,
		userId: userId,
	...
	const { siteName, pages, siteDescription } = await getNL2SiteData(
		intelligenceEndpoint,
		intelligenceApiToken,
		userPrompt,
		sessionId,
	...
	const sitePagesList = pages.map(
		(page: { pageName: string }) => page.pageName,
	);

	stream.progress(NL2PAGE_GENERATING_WEBPAGES);

	// Call NL2Page service to get page content
	telemetry.sendTelemetryEvent(VSCODE_EXTENSION_NL2PAGE_REQUEST, {
		sessionId: sessionId,
		orgId: orgId,
		environmentId: envId,
		userId: userId,
	...
	const sitePages = await getNL2PageData(
		intelligenceEndpoint,
		intelligenceApiToken,
		userPrompt,
		siteName,
...
function previewSitePagesContent(
	...
	try {
		...
		sitePages.forEach((page: any) => {
			sitePagesContent.push({
				name: page.metadata.pageTitle,
				content: page.code,
			});
		});
		...
		sitePagesContent.forEach((page: { name: string; content: string }) => {
			...
			const pageUri = vscode.Uri.joinPath(
				baseUri,
				page.name + HTML_FILE_EXTENSION,
			);
			contentProvider.writeFile(
				pageUri,
				Buffer.from(page.content, UTF8_ENCODING),
			);
		});

		telemetry.sendTelemetryEvent(VSCODE_EXTENSION_PREVIEW_SITE_PAGES, {
			sessionId,
			orgId,
			environmentId: envId,
			userId,
		...
		oneDSLoggerWrapper
			...
			.traceError(
				...
				error as Error,
				{ sessionId, orgId, environmentId: envId, userId },
				{},
			);
		throw error;
	}
