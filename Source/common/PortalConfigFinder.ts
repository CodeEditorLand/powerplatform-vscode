...
export function workspaceContainsPortalConfigFolder(
	workspaceRootFolders: WorkspaceFolder[] | null,
): boolean {
	return (
		workspaceRootFolders?.some((workspaceRootFolder) => {
			return glob.sync("**/website.yml", { cwd: workspaceRootFolder.uri })
				.length;
		}) || false
	);
}

export function getPortalConfigFolderUrl(
	workspaceRootFolders: WorkspaceFolder[] | null,
 ...
 */
export function searchPortalConfigFolder(
...
export function getPortalsOrgURLs(
	workspaceRootFolders: WorkspaceFolder[] | null,
	...
	try {
		workspaceRootFolders?.forEach((workspaceRootFolder) => {
			...
			const manifestFiles = glob.sync("**/*-manifest.yml", {
				dot: true,
		...
		sendTelemetryEvent(telemetry, {
			methodName: getPortalsOrgURLs.name,
			eventName: "getPortalsOrgURLs",
