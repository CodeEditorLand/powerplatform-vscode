...
import type { IOrgDetails } from "../chat-participants/powerpages/PowerPagesChatParticipantTypes";
...
export function handleOrgChangeSuccess(
	...
	extensionContext.globalState.update(ORG_DETAILS_KEY, {
		orgID,
		orgUrl,
		environmentID,
	});
...
export async function initializeOrgDetails(
	...
	const orgDetails: IOrgDetails | undefined =
		extensionContext.globalState.get(ORG_DETAILS_KEY);
	let orgID = "";
	let orgUrl = "";
	let environmentID = "";
	...
	if (
		...
		if (pacActiveOrg && pacActiveOrg.Status === SUCCESS) {
			const orgDetails = handleOrgChangeSuccess(
				pacActiveOrg.Results,
				extensionContext,
			);
			orgID = orgDetails.orgID;
			orgUrl = orgDetails.orgUrl;
