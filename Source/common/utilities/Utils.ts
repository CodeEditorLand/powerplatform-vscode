...
import {
	BAP_API_VERSION,
	BAP_ENVIRONMENT_LIST_URL,
	BAP_SERVICE_ENDPOINT,
	ServiceEndpointCategory,
} from "../services/Constants";
import {
	VSCODE_EXTENSION_GET_BAP_ENDPOINT_UNSUPPORTED_REGION,
	VSCODE_EXTENSION_GET_ENV_LIST_FAILED,
	VSCODE_EXTENSION_GET_ENV_LIST_SUCCESS,
} from "../services/TelemetryConstants";
...
export function showConnectedOrgMessage(
	...
	vscode.window.showInformationMessage(
		vscode.l10n.t({
			message:
				"Power Pages Copilot is now connected to the environment: {0} : {1}",
...
export async function showInputBoxAndGetOrgUrl() {
	return vscode.window.showInputBox({
		...
		prompt: vscode.l10n.t(
			"Active auth profile is not found or has expired. To create a new auth profile, enter the environment URL.",
		),
		ignoreFocusOut: true,
	});
}

export async function showProgressWithNotification<T>(
	title: string,
	task: () => Promise<T>,
): Promise<T> {
...
export function openWalkthrough(extensionUri: vscode.Uri) {
	const walkthroughUri = vscode.Uri.joinPath(
		extensionUri,
		"src",
		"common",
		"copilot",
...
export function getActiveEditorContent(): IActiveFileData {
	...
	const document = activeEditor.document,
		...
		activeFileParams: string[] =
			getLastThreePartsOfFileName(relativeFileName),
	...
	return {
		...
		activeFileParams: {
			dataverseEntity:
				DataverseEntityNameMap.get(activeFileParams[0]) || "",
...
export function checkCopilotAvailability(
	...
	if (!aibEndpoint) {
		...
		// Tenant ID not available in desktop
		sendTelemetryEvent(telemetry, {
...
async function getFileContentByType(
	activeFileUri: vscode.Uri,
	...
	try {
		...
		const files: IRelatedFiles[] = await Promise.all(
			relatedFileTypes.map(async (fileType) => {
				const fileContentResult = await getFileContentByType(
					activeFileUri,
					componentType,
					fileType,
				);
...
export function getECSOrgLocationValue(
	clusterName: string,
		...
		if (envListResponse.ok) {
			...
			oneDSLoggerWrapper
				...
				.traceError(
					VSCODE_EXTENSION_GET_ENV_LIST_FAILED,
					VSCODE_EXTENSION_GET_ENV_LIST_FAILED,
					new Error(envListResponse.statusText),
				);
		}
	} catch (error) {
		sendTelemetryEvent(telemetry, {
...
export function getBAPEndpoint(
