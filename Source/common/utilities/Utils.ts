...
import type { IActiveFileData, IActiveFileParams } from "../copilot/model";
...
export async function createAuthProfileExp(pacWrapper: PacWrapper | undefined) {
	...
	const pacAuthCreateOutput = await showProgressWithNotification(
		vscode.l10n.t(AUTH_CREATE_MESSAGE),
		async () => {
...
export function getActiveEditorContent(): IActiveFileData {
	...
	if (activeEditor) {
		...
		const activeFileParams: string[] =
			getLastThreePartsOfFileName(relativeFileName);

		activeFileData.activeFileContent = document.getText();
		activeFileData.activeFileParams.dataverseEntity =
			DataverseEntityNameMap.get(activeFileParams[0]) || "";
		activeFileData.activeFileParams.entityField =
			EntityFieldMap.get(activeFileParams[1]) || "";
		activeFileData.activeFileParams.fieldType =
			FieldTypeMap.get(activeFileParams[2]) || "";
	}
