...
export class ArtemisService {
	public static async getIntelligenceEndpoint(
		...
		const artemisResponses = await ArtemisService.fetchArtemisResponse(
			orgId,
			telemetry,
			sessionID,
		);
	...
	public static async fetchArtemisResponse(
		orgId: string,
		telemetry: ITelemetry,
		sessionID = "",
	) {
		...
		const artemisResponse = await ArtemisService.fetchIslandInfo(
			endpointDetails,
			telemetry,
			sessionID,
		);
