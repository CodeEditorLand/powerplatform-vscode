...
import {
	CopilotLoginFailureEvent,
	CopilotLoginSuccessEvent,
...
export async function intelligenceAPIAuthentication(
	telemetry: ITelemetry,
	sessionID: string,
	orgId: string,
	firstTimeAuth = false,
): Promise<{ accessToken: string; user: string; userId: string }> {
	let accessToken = "";
	...
	try {
		...
		if (firstTimeAuth) {
			sendTelemetryEvent(telemetry, {
				eventName: CopilotLoginSuccessEvent,
...
export async function graphClientAuthentication(
	...
	try {
		let session = await vscode.authentication.getSession(
			PROVIDER_ID,
			[SCOPE_OPTION_CONTACTS_READ, SCOPE_OPTION_USERS_READ_BASIC_ALL],
			{ silent: true },
		);
...
export function getOIDFromToken(token: string, telemetry: ITelemetry) {
	try {
		...
		sendTelemetryEvent(telemetry, {
			eventName: VSCODE_EXTENSION_DECODE_JWT_TOKEN_FAILED,
			errorMsg: (error as Error).message,
		});
	}
...
export async function powerPlatformAPIAuthentication(
	...
	const PPAPI_WEBSITES_ENDPOINT = [
		ServiceEndpointCategory.TEST,
		ServiceEndpointCategory.PREPROD,
	].includes(serviceEndpointStamp)
		? PPAPI_PREPROD_WEBSITES_SERVICE_SCOPE_DEFAULT
		: PPAPI_WEBSITES_SERVICE_SCOPE_DEFAULT;
