...
} from "../services/AuthenticationProvider";
...
import {
	...
	getNonce,
	getSelectedCode,
	getSelectedCodeLineRange,
	getUserName,
	openWalkthrough,
	showConnectedOrgMessage,
	showInputBoxAndGetOrgUrl,
...
import {
	...
	EXPLAIN_CODE,
	GITHUB_COPILOT_CHAT_EXT,
	PowerPagesParticipantDocLink,
	PowerPagesParticipantPrompt,
	SELECTED_CODE_INFO,
	SELECTED_CODE_INFO_ENABLED,
	sendIconSvg,
	THUMBS_DOWN,
	THUMBS_UP,
	WebViewMessage,
...
import {
	CopilotClearChatEvent,
	CopilotCodeLineCountEvent,
	CopilotCopyCodeToClipboardEvent,
	CopilotExplainCode,
	CopilotExplainCodeSize,
	CopilotInsertCodeToEditorEvent,
	CopilotLoadedEvent,
	CopilotNotAvailableECSConfig,
	CopilotOrgChangedEvent,
	CopilotPanelTryGitHubCopilotClicked,
	CopilotUserFeedbackThumbsDownEvent,
	CopilotUserFeedbackThumbsUpEvent,
	CopilotUserPromptedEvent,
	CopilotWalkthroughEvent,
	VSCodeExtensionGitHubChatNotFound,
	VSCodeExtensionGitHubChatPanelOpened,
} from "./telemetry/telemetryConstants";
import { CESUserFeedback } from "./user-feedback/CESSurvey";
import {
	createAuthProfileExp,
	getDisabledOrgList,
	getDisabledTenantList,
...
export class PowerPagesCopilot implements vscode.WebviewViewProvider {
	...
	constructor(
		...
		this._disposables.push(
			vscode.commands.registerCommand(
				"powerpages.copilot.clearConversation",
				() => {
					if (userName && orgID) {
						sendTelemetryEvent(this.telemetry, {
							eventName: CopilotClearChatEvent,
		...
		if (SELECTED_CODE_INFO_ENABLED) {
			//TODO: Remove this check once the feature is ready
			...
			const handleSelectionChange = async (commandType: string) => {
				...
				if (commandType === EXPLAIN_CODE) {
					...
					sendTelemetryEvent(this.telemetry, {
						eventName: CopilotExplainCodeSize,
						copilotSessionId: sessionID,
						orgId: orgID,
						userId: userID,
						codeLineCount: String(
							selectedCodeLineRange.end -
								selectedCodeLineRange.start,
						),
				...
				this.sendMessageToWebview({
					type: commandType,
					value: {
						start: selectedCodeLineRange.start,
						end: selectedCodeLineRange.end,
						selectedCode: selectedCode,
						tokenSize: withinTokenLimit,
					},
				});
			};
			...
			this._disposables.push(
				vscode.window.onDidChangeTextEditorSelection(() =>
					handleSelectionChange(SELECTED_CODE_INFO),
				),
			...
			this._disposables.push(
				vscode.commands.registerCommand(
					"powerpages.copilot.explain",
					() => {
						sendTelemetryEvent(this.telemetry, {
							eventName: CopilotExplainCode,
							copilotSessionId: sessionID,
							orgId: orgID,
							userId: userID,
						});
						this.show();
						handleSelectionChange(EXPLAIN_CODE);
					},
				),
			);
		}
		this._disposables.push(
			orgChangeEvent((orgDetails: ActiveOrgOutput) =>
				this.handleOrgChangeSuccess(orgDetails),
			),
		);
		...
		if (this._view?.visible) {
			this._disposables.push(
				orgChangeErrorEvent(
					async () => await createAuthProfileExp(this._pacWrapper),
				),
			);
		}
	...
	public async resolveWebviewView(
		...
		webviewView.title =
			vscode.l10n.t(COPILOT_IN_POWERPAGES) +
			(IS_DESKTOP ? "" : " [PREVIEW]");
		...
		if (pacOutput && pacOutput.Status === SUCCESS) {
			...
			await this.handleOrgChangeSuccess({
				OrgId: orgID,
				UserId: userID,
				OrgUrl: activeOrgUrl,
			...
			switch (data.type) {
				case "webViewLoaded": {
					...
					const copilotStrings = {
						EXPLAIN_CODE_PROMPT: vscode.l10n.t(
							"Explain the following code snippet:",
						),
						LARGE_SELECTION: vscode.l10n.t(
							"Selection is too large. Try making a shorter selection.",
						),
						FEATURE_NOT_ENABLED_MESSAGE: vscode.l10n.t(
							"Feature is not enabled for this geo.",
						),
						COPILOT_SUPPORT_MESSAGE: vscode.l10n.t(
						...
						INSERT_CODE_MESSAGE: vscode.l10n.t(
							"Insert code into editor",
						),
						AI_CONTENT_MISTAKES_MESSAGE: vscode.l10n.t(
						...
						HI: vscode.l10n.t("Hi"),
						WELCOME_MESSAGE: vscode.l10n.t(
							"In your own words, describe what you need. You can get help with writing code for Power Pages sites in HTML, CSS, and JS languages.",
						),
						DOCUMENTATION_LINK: vscode.l10n.t(
							'To know more, see <a href="https://go.microsoft.com/fwlink/?linkid=2206366">Copilot in Power Pages documentation.',
						),
						WORKING_ON_IT_MESSAGE:
							vscode.l10n.t("Working on it..."),
						GITHUB_COPILOT_CHAT: vscode.l10n.t(
							'You can use this in <a href="#" id="github-copilot-link">GitHub Copilot with @powerpages</a> and leverage best of both world.',
						),
					...
					if (this.aibEndpoint === COPILOT_UNAVAILABLE) {
						...
						getDisabledTenantList()?.includes(tenantId ?? "")
					) {
						sendTelemetryEvent(this.telemetry, {
							eventName: CopilotNotAvailableECSConfig,
							copilotSessionId: sessionID,
							orgId: orgID,
							userId: userID,
						});
				...
				case "newUserPrompt": {
					sendTelemetryEvent(this.telemetry, {
						eventName: CopilotUserPromptedEvent,
						copilotSessionId: sessionID,
						aibEndpoint: this.aibEndpoint ?? "",
						orgId: orgID,
						userId: userID,
						isSuggestedPrompt: String(data.value.isSuggestedPrompt),
						crossGeoDataMovementEnabledPPACFlag:
							this.crossGeoDataMovementEnabledPPACFlag,
					}); //TODO: Add active Editor info
					orgID
						? (async () => {
								await this.authenticateAndSendAPIRequest(
									data.value.userPrompt,
									orgID,
									this.telemetry,
								);
							})()
				...
				case "insertCode": {
					...
					sendTelemetryEvent(this.telemetry, {
						eventName: CopilotInsertCodeToEditorEvent,
						copilotSessionId: sessionID,
						orgId: orgID,
						userId: userID,
					});
				...
				case "copyCodeToClipboard": {
					...
					sendTelemetryEvent(this.telemetry, {
						eventName: CopilotCopyCodeToClipboardEvent,
						copilotSessionId: sessionID,
						orgId: orgID,
						userId: userID,
					});
				...
				case "userFeedback": {
					...
					if (feedbackValue === THUMBS_UP) {
						sendTelemetryEvent(this.telemetry, {
							eventName: CopilotUserFeedbackThumbsUpEvent,
							copilotSessionId: sessionID,
							orgId: orgID,
							userId: userID,
							subScenario: String(messageScenario),
						});
						CESUserFeedback(
							...
							THUMBS_UP,
							this.telemetry,
							this.geoName as string,
							messageScenario,
							tenantId,
						);
					} else if (feedbackValue === THUMBS_DOWN) {
						sendTelemetryEvent(this.telemetry, {
							eventName: CopilotUserFeedbackThumbsDownEvent,
							copilotSessionId: sessionID,
							orgId: orgID,
							userId: userID,
							subScenario: String(messageScenario),
				...
				case "walkthrough": {
					sendTelemetryEvent(this.telemetry, {
						eventName: CopilotWalkthroughEvent,
						copilotSessionId: sessionID,
						orgId: orgID,
						userId: userID,
					});
				...
				case "codeLineCount": {
					sendTelemetryEvent(this.telemetry, {
						eventName: CopilotCodeLineCountEvent,
						copilotSessionId: sessionID,
						codeLineCount: String(data.value),
						orgId: orgID,
						userId: userID,
					});
					break;
				...
				case "openGitHubCopilotLink": {
					...
					sendTelemetryEvent(this.telemetry, {
						eventName: CopilotPanelTryGitHubCopilotClicked,
						copilotSessionId: sessionID,
						orgId: orgID,
						userId: userID,
					});
					if (
						...
						sendTelemetryEvent(this.telemetry, {
							eventName: VSCodeExtensionGitHubChatPanelOpened,
							copilotSessionId: sessionID,
							orgId: orgID,
							userId: userID,
						});
						...
						sendTelemetryEvent(this.telemetry, {
							eventName: VSCodeExtensionGitHubChatNotFound,
							copilotSessionId: sessionID,
							orgId: orgID,
							userId: userID,
						});
	...
	private async handleLogin() {
		...
		if (pacOutput && pacOutput.Status === SUCCESS) {
			...
			intelligenceAPIAuthentication(
				...
				sessionID,
				orgID,
			).then(({ accessToken, user, userId }) => {
				this.intelligenceAPIAuthenticationHandler.call(
					this,
					accessToken,
					user,
					userId,
			...
			const pacAuthCreateOutput = await showProgressWithNotification(
				AUTH_CREATE_MESSAGE,
				async () => {
					return await this._pacWrapper?.authCreateNewAuthProfileForOrg(
						userOrgUrl,
					);
				},
			);
	...
	private async authenticateAndSendAPIRequest(
		data: UserPrompt[],
		orgID: string,
		...
		return intelligenceAPIAuthentication(telemetry, sessionID, orgID)
			.then(async ({ accessToken, user, userId }) => {
				...
				const { activeFileContent, activeFileParams } =
					getActiveEditorContent();
				...
				if (
					activeFileParams.dataverseEntity == ADX_ENTITYFORM ||
					activeFileParams.dataverseEntity == ADX_ENTITYLIST
				) {
					metadataInfo = await getEntityName(
						telemetry,
						sessionID,
						activeFileParams.dataverseEntity,
					);

					const dataverseToken = (
						await dataverseAuthentication(
							telemetry,
							activeOrgUrl,
							true,
						)
					...
					if (activeFileParams.dataverseEntity == ADX_ENTITYFORM) {
						const formColumns = await getFormXml(
							metadataInfo.entityName,
							metadataInfo.formName,
							activeOrgUrl,
							dataverseToken,
							telemetry,
							sessionID,
	...
	private async handleOrgChangeSuccess(activeOrg: ActiveOrgOutput) {
		...
		sendTelemetryEvent(this.telemetry, {
			eventName: CopilotOrgChangedEvent,
			copilotSessionId: sessionID,
			orgId: orgID,
		});

		const intelligenceAPIEndpointInfo =
			await ArtemisService.getIntelligenceEndpoint(
				orgID,
				this.telemetry,
				sessionID,
				environmentId,
		...
		const copilotAvailabilityStatus = checkCopilotAvailability(
			this.aibEndpoint,
			orgID,
			this.telemetry,
			sessionID,
	...
	private async intelligenceAPIAuthenticationHandler(
		accessToken: string,
		user: string,
		userId: string,
	) {
        ...
        <body>
          <div class="copilot-window">
            ...
            <div class="chat-input" id="input-component">
              ...
              <p class="disclaimer">${vscode.l10n
					.t(`Make sure AI-generated content is accurate and appropriate before using. <a href="https://go.microsoft.com/fwlink/?linkid=2240145">Learn more</a> | <a href="https://go.microsoft.com/fwlink/?linkid=2189520">View
              terms</a>`)}</p>
            </div>
          </div>
