...
import {
	...
	CopilotGetEntityFailureEvent,
	CopilotYamlParsingFailureEvent,
} from "./telemetry/telemetryConstants";

declare const IS_DESKTOP: string | undefined;

export async function getEntityColumns(
	entityName: string,
	orgUrl: string,
	apiToken: string,
	...
	try {
		...
		}api/data/v9.2/EntityDefinitions(LogicalName='${entityName}')?$expand=Attributes`;
...
export async function getFormXml(
	entityName: string,
	formName: string,
	orgUrl: string,
	apiToken: string,
...
function getAttributesFromResponse(jsonResponse: any): string[] {
	if (
		...
		Array.isArray(jsonResponse.Attributes) &&
	...
	try {
		...
		if (activeEditor) {
			...
			const matchingFiles = await getMatchingFiles(
				...
				fileNameFirstPart,
			); // ["Copilot-Student-Loan-Registration-56a4.basicform.yml"]
		...
		sendTelemetryEvent(telemetry, {
			eventName: CopilotGetEntityFailureEvent,
			copilotSessionId: sessionID,
...
function parseYamlContent(
	...
	try {
		...
		sendTelemetryEvent(telemetry, {
			eventName: CopilotYamlParsingFailureEvent,
			copilotSessionId: sessionID,
			dataverseEntity: dataverseEntity,
			error: error as Error,
		});
