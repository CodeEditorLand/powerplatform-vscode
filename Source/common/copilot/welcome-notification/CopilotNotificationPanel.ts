...
export async function copilotNotificationPanel(
	...
	NotificationPanel.webview.html = getWebviewContent(
		notificationCssUri,
		notificationJsUri,
		copilotImageUri,
		arrowImageUri,
		...
		async (message) => {
			switch (message.command) {
				...
				case "tryCopilot":
					...
					break;
				case "learnMore":
					telemetry.sendTelemetryEvent(CopilotWalkthroughEvent, {
						listOfOrgs: telemetryData,
						countOfActivePortals: countOfActivePortals as string,
					});
...
function getWebviewURIs(
	...
	const notificationCssPath = vscode.Uri.joinPath(
		srcPath,
	...
	const notificationJsPath = vscode.Uri.joinPath(
		srcPath,
	...
	return `<!DOCTYPE html>
      ...
      <head>
          ...
          <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${
				webview.cspSource
			}; img-src ${webview.cspSource}; script-src 'nonce-${nonce}';">
      ...
      <div class="container">
        ...
        <p id="welcome-text">${vscode.l10n.t(
        ...
        <a href="#" class="walkthrough-content" id="walkthroughLink"> <span id="walk-text">${vscode.l10n.t(
			"Learn more about Copilot",
		)} </span> <img src="${arrowImageUri}" id="arrow-icon"> </a>
        </div>
        <div>
