...
import {
	CopilotDisableList,
	EnablePowerPagesGitHubCopilot,
	EnableProDevCopilot,
} from "../../ecs-features/ecsFeatureGates";
import {
...
export function getDisabledOrgList() {
	const disallowedProDevCopilotOrgs =
		ECSFeaturesClient.getConfig(
			CopilotDisableList,
		).disallowedProDevCopilotOrgs;

	if (
		disallowedProDevCopilotOrgs === undefined ||
		disallowedProDevCopilotOrgs === ""
	) {
		return [];
	}
...
export function getDisabledTenantList() {
	const disallowedProDevCopilotTenants =
		ECSFeaturesClient.getConfig(
			CopilotDisableList,
		).disallowedProDevCopilotTenants;

	if (
		disallowedProDevCopilotTenants === undefined ||
		disallowedProDevCopilotTenants === ""
	) {
		return [];
	}
...
export function isCopilotSupportedInGeo() {
	const supportedGeoList =
		ECSFeaturesClient.getConfig(
			EnableProDevCopilot,
		).capiSupportedProDevCopilotGeoList;
...
export function enableCrossGeoDataFlowInGeo() {
	const enableCrossGeoDataFlowInGeo =
		ECSFeaturesClient.getConfig(
			EnableProDevCopilot,
		).capiSupportedProDevCopilotGeoWithCrossGeoDataFlow;

	if (
		enableCrossGeoDataFlowInGeo === undefined ||
		enableCrossGeoDataFlowInGeo === ""
	) {
		return [];
	}
...
export function isPowerPagesGitHubCopilotEnabled() {
	const enablePowerpagesInGithubCopilot = ECSFeaturesClient.getConfig(
		EnablePowerPagesGitHubCopilot,
	).enablePowerpagesInGithubCopilot;
