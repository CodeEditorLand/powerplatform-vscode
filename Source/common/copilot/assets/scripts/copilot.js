...
(function script() {
	...
	function parseCodeBlocks(responseText, isUserCode) {
		...
		for (let i = 0; i < responseLength; i++) {
			...
			if (
				responseText[i].displayText ===
				copilotStrings.FEATURE_NOT_ENABLED_MESSAGE
			) {
			...
			isUserCode
				? codeDiv.classList.add("user-code")
				: codeDiv.classList.add("copilot-code");

			const codeBlock = responseText[i].code;
...
`;
        ...
        <button id="loginButton" >${copilotStrings.LOGIN_BUTTON}</button>`;
			...
			userLoggedIn: () => {
				const notLoggedInDiv =
					messageElement.querySelector(".not-loggedIn");
				if (notLoggedInDiv) {
				...
				suggestedPrompts.forEach((suggestedPrompt) => {
					suggestedPrompt.addEventListener(
						"click",
				...
				walkthroughContent.addEventListener(
					"click",
	...
	window.addEventListener("message", (event) => {
		...
		switch (message.type) {
			...
			case "selectedCodeInfo": {
				const chatInputLabel =
					document.getElementById("input-label-id");
				...
				chatInputLabel.innerText = `Lines ${
					message.value.start + 1
				} - ${message.value.end + 1} selected`;
				break;
			}
			case "explainCode": {
				selectedCode = message.value.selectedCode;
	...
	function handleThumbsUpClick(element) {
		...
		const messageId = element.closest(".message-wrapper").dataset.id;
		...
		const thumbsDownPath =
			element.parentNode.querySelector("#thumbsdown-path");
	...
	function updateInputBoxValue() {
		if (currentIndex >= 0) {
			chatInput.value =
				inputHistory[inputHistory.length - 1 - currentIndex];
		} else {
			chatInput.value = "";
		}
	}
