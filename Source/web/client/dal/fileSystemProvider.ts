...
import {
	getImageFileContent,
	getRangeForMultilineMatch,
	isImageFileSupportedForEdit,
	...
	async writeFile(
		...
		if (
			...
			if (isImageEdit) {
				...
				updateFileContentInFileDataMap(
					uri.fsPath,
					getImageFileContent(uri.fsPath, content),
					true,
				);
			}
		...
		if (
			...
			WebExtensionContext.telemetry.sendInfoTelemetry(
				webExtensionTelemetryEventNames.WEB_EXTENSION_VSCODE_START_COMMAND,
				{
					...
					entityName: WebExtensionContext.defaultEntityType,
					isMultifileEnabled:
						WebExtensionContext.showMultifileInVSCode.toString(),
					duration: (
						new Date().getTime() -
						WebExtensionContext.extensionActivationTime
					).toString(),
				},
			);
		}
