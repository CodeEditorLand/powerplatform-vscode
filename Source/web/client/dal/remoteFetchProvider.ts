...
import type { IAttributePath, IFileInfo } from "../common/interfaces";
import {
	EntityMetadataKeyCore,
	type SchemaEntityMetadata,
	folderExportType,
	schemaEntityKey,
	schemaEntityName,
	schemaKey,
...
import {
	getEntityNameForExpandedEntityContent,
	getRequestUrlForEntities,
} from "../utilities/folderHelperUtility";
import {
	encodeAsBase64,
	getAttributePath,
	getEntity,
...
import {
	getCustomRequestURL,
	getMappingEntityContent,
	getMappingEntityId,
	getMetadataInfo,
	getMimeType,
	getRequestURL,
} from "../utilities/urlBuilderUtil";
import type { PortalsFS } from "./fileSystemProvider";
...
export async function fetchDataFromDataverseAndUpdateVFS(
	...
	try {
		const entityRequestURLs = getRequestUrlForEntities(
			defaultFileInfo?.entityId,
			defaultFileInfo?.entityName,
		);
		...
		await Promise.all(
			entityRequestURLs.map(async (entity) => {
				...
				if (defaultFileInfo === undefined) {
					// This will be undefined for bulk entity load
		...
		WebExtensionContext.telemetry.sendErrorTelemetry(
			telemetryEventNames.WEB_EXTENSION_FAILED_TO_PREPARE_WORKSPACE,
...
async function createContentFiles(
	...
	try {
		...
		filePathInPortalFS =
			filePathInPortalFS ??
			`${Constants.PORTALS_URI_SCHEME}:/${portalFolderName}/${subUri}/`;
		if (exportType && exportType === folderExportType.SubFolders) {
			filePathInPortalFS = `${filePathInPortalFS}${getSanitizedFileName(
				fileName,
			)}/`;
		...
		if (
			defaultFileInfo?.fileName === undefined &&
		...
		const rootWebPageIdAttribute = entityDetails?.get(
			schemaEntityKey.ROOT_WEB_PAGE_ID,
		);

		await processDataAndCreateFile(
			attributeArray,
			attributeExtension,
			entityName,
			result,
...
async function processDataAndCreateFile(
	...
	for (counter; counter < attributeArray.length; counter++) {
		...
		if (fileExtension === undefined) {
			const expandedContent = getAttributeContent(
				result,
				attributePath,
				entityName,
				entityId,
			...
			let fileNameWithExtension = GetFileNameWithExtension(
				entityName,
				fileName,
				languageCode,
				fileExtension,
	...
	if (
		...
		// Triggered default file load defines this value
...
async function createFile(
	...
	const base64Encoded: boolean = isBase64Encoded(entityName, attribute);
	...
	const isPreloadedContent = mappingEntityFetchQuery
		? isWebfileContentLoadNeeded(fileNameWithExtension, fileUri)
		: true;

	// update func for webfiles for V2
	const attributePath: IAttributePath = getAttributePath(attribute);
	...
	if (mappingEntityFetchQuery && isPreloadedContent) {
		...
		fileContent = getAttributeContent(
			result,
			attributePath,
			entityName,
			entityId,
	...
	const entityMetadata = getMetadataInfo(
		result,
		metadataKeys.filter((key) => key !== undefined) as string[],
	);
...
async function fetchMappingEntityContent(
	...
	if (
		isPortalVersionV1() &&
		result[Constants.ODATA_COUNT] > 0 &&
		data.length > 0
	) {
		return data[0];
	}
...
export async function preprocessData(
	...
	try {
		...
		if (
			entityType === schemaEntityName.ADVANCEDFORMS &&
			schema.toLowerCase() ===
			...
			const fetchedFileId = entityDetails?.get(
				schemaEntityKey.FILE_ID_FIELD,
			);
			const formsData = await fetchFromDataverseAndCreateFiles(
				entityType,
				getCustomRequestURL(dataverseOrgUrl, entityType),
			);
			const attributePath: IAttributePath = getAttributePath(
			...
			data?.forEach((dataItem: any) => {
				const webFormSteps = getAttributeContent(
					dataItem,
					attributePath,
					entityType,
					fetchedFileId as string,
				) as [];
			...
			WebExtensionContext.telemetry.sendInfoTelemetry(
				telemetryEventNames.WEB_EXTENSION_PREPROCESS_DATA_SUCCESS,
