...
import type { IOrgInfo } from "../../common/copilot/model";
import {
	copilotNotificationPanel,
	disposeNotificationPanel,
} from "../../common/copilot/welcome-notification/CopilotNotificationPanel";
import {
	PowerPagesAppName,
	PowerPagesClientName,
} from "../../common/ecs-features/constants";
import { ECSFeaturesClient } from "../../common/ecs-features/ecsFeatureClient";
import { ArtemisService } from "../../common/services/ArtemisService";
import type { IArtemisAPIOrgResponse } from "../../common/services/Interfaces";
...
import type { IEntityInfo } from "./common/interfaces";
import { PortalsFS } from "./dal/fileSystemProvider";
import { NPSService } from "./services/NPSService";
import { telemetryEventNames } from "./telemetry/constants";
import type { WebExtensionTelemetry } from "./telemetry/webExtensionTelemetry";
import type { IPortalWebExtensionInitQueryParametersTelemetryData } from "./telemetry/webExtensionTelemetryInterface";
import { sendingMessageToWebWorkerForCoPresence } from "./utilities/collaborationUtils";
import {
	getEnvironmentIdFromUrl,
	isCoPresenceEnabled,
	updateFileContentInFileDataMap,
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (args) => {
				...
				WebExtensionContext.telemetry.sendInfoTelemetry(
					telemetryEventNames.WEB_EXTENSION_ORG_GEO,
					{ orgId: orgId, orgGeo: geoName },
				);
...
export function enableTextSearchFunctionality(
	context: vscode.ExtensionContext,
	portalsFS: PortalsFS,
) {
	const textSearchProvider = vscode.workspace.registerTextSearchProvider(
		PORTALS_URI_SCHEME,
		{
...
export function enableFileSearchFunctionality(
	...
	const fileSearchProvider = vscode.workspace.registerFileSearchProvider(
		PORTALS_URI_SCHEME,
		{
...
export function powerPagesNavigation() {
	...
	WebExtensionContext.telemetry.sendInfoTelemetry(
		telemetryEventNames.WEB_EXTENSION_POWER_PAGES_WEB_VIEW_REGISTERED,
	);
}
			...
			event.opened.concat(event.changed).forEach((tab) => {
				if (
					...
					const entityInfo: IEntityInfo = {
						...
						rootWebPageId: getFileRootWebPageId(
							document.uri.fsPath,
						),
					};
...
export function processActiveTextEditorChange(
	context: vscode.ExtensionContext,
) {
	context.subscriptions.push(
...
export function processWillStartCollaboration(
	context: vscode.ExtensionContext,
) {
...
export function createWebWorkerInstance(context: vscode.ExtensionContext) {
	try {
		...
		WebExtensionContext.getWorkerScript(workerUrl).then((workerScript) => {
			...
			if (WebExtensionContext.worker !== undefined) {
				WebExtensionContext.worker.onmessage = (event) => {
					...
					if (
						data.type ===
						Constants.workerEventMessages.REMOVE_CONNECTED_USER
					) {
					...
					if (
						...
						if (data.currentConnectionId) {
							WebExtensionContext.setCurrentConnectionId(
			...
			const entityInfo = context.workspaceState.get(
				vscode.window.activeTextEditor?.document.uri.fsPath as string,
			) as IEntityInfo;
			if (
				entityInfo.rootWebPageId === undefined ||
				entityInfo.rootWebPageId === "" ||
				entityInfo.rootWebPageId === " "
			) {
				entityInfo.rootWebPageId = getFileRootWebPageId(
		...
		WebExtensionContext.telemetry.sendInfoTelemetry(
			telemetryEventNames.WEB_EXTENSION_WEB_WORKER_REGISTERED,
		);
	} catch (error) {
		WebExtensionContext.telemetry.sendErrorTelemetry(
...
export function registerCopilot(context: vscode.ExtensionContext) {
	try {
		const orgInfo = {
			...
			environmentName: "",
			activeOrgUrl: WebExtensionContext.urlParametersMap.get(
				queryParameters.ORG_URL,
			) as string,
			tenantId: WebExtensionContext.urlParametersMap.get(
				queryParameters.TENANT_ID,
			) as string,
		} as IOrgInfo;

		const copilotPanel = new copilot.PowerPagesCopilot(
			context.extensionUri,
			context,
			WebExtensionContext.telemetry.getTelemetryReporter(),
			undefined,
		...
		context.subscriptions.push(
			vscode.window.registerWebviewViewProvider(
				copilot.PowerPagesCopilot.viewType,
				copilotPanel,
				{
					webviewOptions: {
...
function showNotificationForCopilot(
	...
	if (
		vscode.workspace
			...
			.get("experimental.enableWebCopilot") === false
	) {
		return;
	}

	const isCopilotNotificationDisabled = context.globalState.get(
		COPILOT_NOTIFICATION_DISABLED,
		false,
	);
	if (!isCopilotNotificationDisabled) {
		WebExtensionContext.telemetry.sendInfoTelemetry(
			telemetryEventNames.WEB_EXTENSION_WEB_COPILOT_NOTIFICATION_SHOWN,
			{ orgId: orgId },
		);
...
async function fetchArtemisData(
	orgId: string,
): Promise<IArtemisAPIOrgResponse> {
	const artemisResponse = await ArtemisService.fetchArtemisResponse(
		orgId,
		WebExtensionContext.telemetry.getTelemetryReporter(),
	);
	if (artemisResponse === null || artemisResponse.length === 0) {
		...
		return {
			geo: "",
			geoLongName: "",
		} as unknown as IArtemisAPIOrgResponse;
	}
...
function logOneDSLogger(queryParamsMap: Map<string, string>) {
	...
	if (
		queryParamsMap.has(queryParameters.ENTITY) &&
		queryParamsMap.has(queryParameters.ENTITY_ID)
	) {
