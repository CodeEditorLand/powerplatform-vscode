...
} from "../../../common/OneDSLoggerTelemetry/web/client/webExtensionTelemetryInterface";
import type { AppInsightsResource } from "../../../common/pp-tooling-telemetry-node/AppInsightsResource";
...
export class WebExtensionTelemetry {
	...
	public sendExtensionInitPathParametersTelemetry(
		...
		if (entity && entityId) {
			(telemetryData.properties.entity =
				this.getPathParameterValue(entity)),
				(telemetryData.properties.entityId =
					this.getPathParameterValue(entityId));
		}
	...
	public sendExtensionInitQueryParametersTelemetry(
		...
		const telemetryData: IPortalWebExtensionInitQueryParametersTelemetryData =
			{
				...
				properties: {
					...
					siteVisibility: queryParamsMap.get(
					...
					referrerSource: queryParamsMap.get(
						queryParameters.REFERRER_SOURCE,
					),
	...
	public sendAPITelemetry(
		...
		eventName =
			eventName ??
			webExtensionTelemetryEventNames.WEB_EXTENSION_API_REQUEST;
	...
	public sendAPIFailureTelemetry(
		...
		this.sendAPITelemetry(
			...
			errorMessage,
			isNullOrUndefined(eventName)
				? webExtensionTelemetryEventNames.WEB_EXTENSION_API_REQUEST_FAILURE
				: eventName,
			status,
		);
	}
