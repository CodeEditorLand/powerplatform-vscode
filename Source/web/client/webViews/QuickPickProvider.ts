...
export class QuickPickProvider {
	...
	public async updateQuickPickItems(entityInfo: IEntityInfo) {
		...
		for (const [, value] of connectedUsersMap.entries()) {
			for (const connection of value._connectionData) {
				...
				if (
					...
					const contentPageId =
						WebExtensionContext.entityForeignKeyDataMap.getEntityForeignKeyMap.get(
							`${connection.entityId[0]}`,
						);

					// if content is localized, then check for the content page id
					if (
						connection.entityId[0] === entityInfo.entityId ||
						(contentPageId &&
		...
		this.items = currentUsers.length
			? currentUsers
			: [
					{
	...
	private getLength(): number {
		if (
			this.items.length === 1 &&
			this.items[0].label ===
				Constants.WEB_EXTENSION_QUICK_PICK_DEFAULT_STRING
		) {
			return 0;
		}
	...
	public async showQuickPick() {
		const selectedUser = await vscode.window.showQuickPick(this.items, {
			title: vscode.l10n.t(
				Constants.WEB_EXTENSION_QUICK_PICK_TITLE.toUpperCase() +
					` (${this.getLength()})`,
			),
	...
	public async collaborationQuickPick(selectedOption: IQuickPickItem) {
		...
		const collaborationOptionsSelected = await vscode.window.showQuickPick(
			collaborationOptions,
			{
				title: vscode.l10n.t(
					Constants.WEB_EXTENSION_COLLABORATION_OPTIONS_CONTACT.toUpperCase() +
		...
		if (collaborationOptionsSelected) {
			if (
				collaborationOptionsSelected.label ===
				Constants.START_TEAMS_CHAT
			) {
				if (selectedOption.id) {
					WebExtensionContext.openTeamsChat(selectedOption.id);
				} else {
					vscode.window.showInformationMessage(
						Constants.WEB_EXTENSION_TEAMS_CHAT_NOT_AVAILABLE,
					);
				}
			} else if (
				collaborationOptionsSelected.label === Constants.SEND_AN_EMAIL
			) {
				if (selectedOption.id) {
					WebExtensionContext.openMail(selectedOption.id);
				} else {
					vscode.window.showInformationMessage(
						Constants.WEB_EXTENSION_SEND_EMAIL_NOT_AVAILABLE,
					);
				}
			}
		}
