			...
			for (const [userId, member] of members.entries()) {
				...
				if (
					...
					return {
						...
						aadObjectId: member.additionalDetails.AadObjectId,
					};
				}
			}

			throw new Error(
				"Web Extension WebWorker GetUserIdByConnectionId Failed",
			);
		};
				...
				audience.getMembers().forEach(async (member) => {
					try {
						...
						userConnections.forEach((connection) => {
							userConnectionData.push({
								connectionId: connection.id,
								entityId: connectionIdInContainer.get(
									connection.id,
								),
							});
						});
						...
						self.postMessage({
							...
							eventName:
								"webExtensionContainerInitialPopulateFailed",
			...
			if (!existingMembers.get(member.additionalDetails.AadObjectId)) {
				self.postMessage({
					...
					removeConnectionData: {
						...
						entityId: entityInfo.rootWebPageId,
					},
				});
				self.postMessage({
					type: "telemetry-info",
				...
				self.postMessage({
					...
					errorMessage:
						"Web Extension WebWorker Member Removed Failed",
				});
			}
		});
		...
		selectionSharedMap.on("valueChanged", async (changed, local) => {
			try {
				...
				userConnections.forEach((connection) => {
					userConnectionData.push({
						connectionId: connection.id,
						entityId: connectionIdInContainer.get(connection.id),
					});
				});
				...
				self.postMessage({
					...
					methodName: "webWorker valueChanged",
					eventName:
						"webExtensionWebWorkerGetUserIdByConnectionIdFailed",
					errorMessage: error?.message,
					error: error,
				});
			}
