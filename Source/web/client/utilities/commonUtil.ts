...
export function convertContentToUint8Array(
	content: string,
	isBase64Encoded: boolean,
): Uint8Array {
	return isBase64Encoded
		? new Uint8Array(Buffer.from(content, BASE_64))
...
export function isExtensionNeededInFileName(entity: string) {
	return (
		entity === schemaEntityName.WEBTEMPLATES ||
...
export function getAttributeContent(
	...
	try {
		if (result[attributePath.source] && attributePath.relativePath.length) {
			value = JSON.parse(result[attributePath.source])[
...
export function isWebfileContentLoadNeeded(
	fileName: string,
	fsPath: string,
): boolean {
	...
	WebExtensionContext.telemetry.sendInfoTelemetry(
		webExtensionTelemetryEventNames.WEB_EXTENSION_WEBFILE_EXTENSION,
...
export function isPortalVersionV1(): boolean {
	return (
		WebExtensionContext.currentSchemaVersion.toLowerCase() ===
...
export function getBackToStudioURL() {
	const region = WebExtensionContext.urlParametersMap.get(
		queryParameters.REGION,
	) as string;

	if (
		isStringUndefinedOrEmpty(
			WebExtensionContext.urlParametersMap.get(queryParameters.ENV_ID),
		) ||
		isStringUndefinedOrEmpty(
			WebExtensionContext.urlParametersMap.get(queryParameters.REGION),
		) ||
		isStringUndefinedOrEmpty(
			WebExtensionContext.urlParametersMap.get(
	...
	return BACK_TO_STUDIO_URL_TEMPLATE.replace(
		"{environmentId}",
		...
		.replace(
			...
			region.toLowerCase() === STUDIO_PROD_REGION
				? ""
				: `.${WebExtensionContext.urlParametersMap.get(queryParameters.REGION) as string}`,
		)
		.replace(
			"{webSiteId}",
			WebExtensionContext.urlParametersMap.get(
...
export function getSupportedImageFileExtensionsForEdit() {
	return [
		...
		"jpe",
		"tpic",
	]; // Luna paint supported image file extensions
}
...
export function isImageFileSupportedForEdit(fileName: string): boolean {
	...
	const isSupported =
		fileExtension !== undefined
			? supportedImageFileExtensions.includes(fileExtension.toLowerCase())
			: false;

	if (isSupported) {
		WebExtensionContext.telemetry.sendInfoTelemetry(
			webExtensionTelemetryEventNames.WEB_EXTENSION_IMAGE_EDIT_SUPPORTED_FILE_EXTENSION,
			{ fileExtension: fileExtension },
		);
	}
...
export function updateFileContentInFileDataMap(
	fileFsPath: string,
	fileContent: string | Uint8Array,
	isFileContentBase64Encoded = false,
) {
	const fileData = WebExtensionContext.fileDataMap.getFileMap.get(fileFsPath);

	// Update the latest content in context
	if (fileData?.entityId && fileData.attributePath) {
		fileContent = convertContentToString(
			fileContent,
			isFileContentBase64Encoded
				? false
...
export function getImageFileContent(
	fileFsPath: string,
	fileContent: Uint8Array,
) {
	const webFileV2 = isWebFileV2(
		getFileEntityName(fileFsPath),
		getFileAttributePath(fileFsPath)?.source,
	);
...
export function getRangeForMultilineMatch(
	text: string,
	pattern: string,
	index: number,
) {
	const textBeforePattern = text.substring(0, index).split("\n");
	const textTillPattern = text
		.substring(0, index + pattern.length)
		.split("\n");
...
export async function validateWebsitePreviewURL(): Promise<boolean> {
	const envId = WebExtensionContext.urlParametersMap?.get(
		queryParameters.ENV_ID,
	);
	const serviceEndpointStamp = WebExtensionContext.serviceEndpointCategory;
	const websitePreviewId = WebExtensionContext.urlParametersMap?.get(
		queryParameters.PORTAL_ID,
	);

	if (
		serviceEndpointStamp === ServiceEndpointCategory.NONE ||
		!envId ||
		!websitePreviewId
	) {
		WebExtensionContext.telemetry.sendErrorTelemetry(
	...
	const siteDetails = await PPAPIService.getWebsiteDetailsById(
	...
	return (
		siteDetails.websiteUrl.length !== 0 &&
		WebExtensionContext.urlParametersMap.get(
			queryParameters.WEBSITE_PREVIEW_URL,
		) !== undefined &&
		siteDetails.websiteUrl.toLocaleLowerCase().trim() ===
			WebExtensionContext.urlParametersMap
				.get(queryParameters.WEBSITE_PREVIEW_URL)
				?.toLocaleLowerCase()
				.trim()
	);
}
