...
export function getAttributeContent(
	...
	try {
		...
		WebExtensionContext.telemetry.sendErrorTelemetry(
			telemetryEventNames.WEB_EXTENSION_ATTRIBUTE_CONTENT_ERROR,
			getAttributeContent.name,
			`For ${entityName} with entityId ${entityId} and attributePath ${JSON.stringify(
				attributePath,
			)} error: ${errorMsg}`,
		);
	}
...
export function setFileContent(
	...
	try {
		...
		const errorMsg = (error as Error)?.message;
		WebExtensionContext.telemetry.sendErrorTelemetry(
			telemetryEventNames.WEB_EXTENSION_SET_FILE_CONTENT_ERROR,
			setFileContent.name,
			errorMsg,
		);
	}
...
export function isWebfileContentLoadNeeded(
	...
	WebExtensionContext.telemetry.sendInfoTelemetry(
		telemetryEventNames.WEB_EXTENSION_WEBFILE_EXTENSION,
...
export function isImageFileSupportedForEdit(fileName: string): boolean {
	...
	if (isSupported) {
		WebExtensionContext.telemetry.sendInfoTelemetry(
			telemetryEventNames.WEB_EXTENSION_IMAGE_EDIT_SUPPORTED_FILE_EXTENSION,
			{ fileExtension: fileExtension },
		);
	}
